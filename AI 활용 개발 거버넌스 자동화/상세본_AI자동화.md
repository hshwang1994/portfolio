...
> | **Closed-loop** | 결과가 다시 입력으로 돌아오는 구조. *사고 → 기록 → 규칙 반영 → 자동 차단 → 또 발견* 이 끊김 없이 돌면서 **쓸수록 똑똑해집니다**. |
...
| **문제** | 여러 고객사 단일 코드베이스에서 AI가 만든 변경이 고객사 경계, 외부 시스템 기준, 컨벤션을 깨면 다른 고객사 배포로 전파 |
| **핵심 설계** | "왜 막나(규칙, 34종)" / "무엇을 막나(정책, 12종)" / "어떻게 막나(검사 스크립트, 29개)"를 세 계층으로 분리. 6단계 통제 + Closed-loop |

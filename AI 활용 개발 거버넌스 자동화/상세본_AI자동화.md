...
|------|--------------------------------|
| 스케줄러 안전장치 누락 | 예약 실행 코드 42개 중 31개에 오류 보호 블록이 없어 실패가 조용히 묻히던 상태(보호 적용은 11개뿐) → 어노테이션 위치와 본문의 보호 블록 유무를 자동 판정하는 검사로 변환 → 이후 새 예약 코드는 누락이 자동 검출 |
...
| 계층 | 자산 | 자료 형식 | 수량 |
|------|------|----------|------|
| A | 기록, 보관 (사례, 결정, 점검 이력) | .md/.yaml (append-only) | 누적: 사례 108, 결정 기록 16 / 시계열: 점검 보고서 39, 스냅샷 28, 회차 15 |

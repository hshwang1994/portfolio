...
> | **Closed-loop** | 발견된 오류를 규칙으로 기록해, 이후 같은 패턴이 자동 차단되는 되먹임 구조. *사고 발견 → 원인 분석 → 검사 규칙으로 전환 → 다음 변경부터 자동 검출* 이 돌면서 검사 규칙이 쌓입니다. |
> | **git hook** | git이 *저장(commit), 병합(merge)* 같은 특정 순간에 **자동으로 실행하는 검사 스크립트**. |
> | **advisory(경고형)** | 막지 않고 **경고만** 하는 검사. |
> | **강제 차단(blocking)** | 위반 시 저장(커밋)이나 머지 자체를 막는 검사. 반드시 고친 뒤 다시 시도해야 합니다. |
> | **append-only** | **추가만** 되고 수정, 삭제는 막은 기록 방식. 과거 근거가 사라지지 않습니다. |
> | **회귀(regression)** | 새 변경 때문에 **기존 기능이 깨지는 것**. |
>
> 수치는 별도 표기가 없으면 2026-05-29, 작업 브랜치 기준 **코드에서 직접 센 실측값**입니다. 시계열 항목(점검 보고서, 스냅샷, 회차 기록)은 측정 시점에 따라 **증감**합니다 — 스냅샷·회차는 머지마다 늘고, 점검 보고서는 정리·삭제로 줄기도 합니다.
...
| 구분 | 내용 |
|------|------|
| **포지션** | 개발 생산성, 플랫폼 엔지니어링 성격의 사내 도구. Java/Spring 백엔드 개발 업무를 하면서, AI가 만든 코드를 자동 검증하는 영역을 단독 설계, 구현 |
| **문제** | 여러 고객사가 단일 코드베이스를 공유하는 환경에서, AI가 만든 변경이 고객사 경계, 외부 시스템 연동 기준, 코드 컨벤션을 깨면 다른 고객사 배포로 전파 |
| **핵심 설계** | 검증을 세 계층으로 분리 — 규칙(왜 막나: 의도·근거, 34종) / 정책(무엇을 기준으로: 고객사·경계 데이터, 12종) / 검사 스크립트(어떻게 막나: 자동 실행, 29개). 6단계 통제 + 발견된 오류를 되돌리는 Closed-loop |
| **대표 결과** | 누적 108건 사례를 자동 검출 자산으로 전환. 비밀값과 보호 경로 침범은 저장 시점 차단, 고객사 경계 위반은 자동 감지(경고), 합친 뒤 7종 자동 점검 |
...
이 제품은 OpenStack, VMware, Kubernetes, Ansible을 통합 관리하는 **프라이빗 클라우드 플랫폼**입니다. 같은 코드베이스를 여러 고객사(고객사 A~F)가 함께 씁니다. 고객사별로 다른 부분만 별도 모듈로 끼워 넣는 구조이고, 고객사마다 자원 관리 방식과 외부 시스템 연동 기준이 다릅니다.
...
프라이빗 클라우드 제품이라, 고객사 도메인이 달라질 때마다 **자원을 쓰는 목적, 과금 구조, 외부 시스템(Jenkins, VMware, GitLab 등) 연동 기준**이 달라집니다. 그런데 이들 고객사가 **하나의 코드베이스**를 함께 봅니다. 이런 환경에서 AI가 만든 변경을 무작정 반영하면:

- 한 고객사용 분기가 공통 코드에 섞이면 다른 고객사들 배포로 그대로 전파됩니다.
- 외부 시스템 실제 동작과 어긋난 코드가 들어가면 운영 사고로 노출됩니다.
- 회사 컨벤션을 안 따르면 다른 사람이 유지보수하기 어려워집니다.
- 예전에 겪은 오류도 사람 기억에만 기대면 다시 들어옵니다.

즉 생산성을 올리려 넣은 AI가, 통제 장치 없이는 오히려 여러 고객사 전체로 사고를 퍼뜨리는 통로가 됩니다.

### 해법 — 검사로 끝나지 않고, 결과를 되먹이는 거버넌스

그래서 단순히 "AI에게 규칙을 알려주는 설정 묶음"에 그치지 않고, **발견된 오류를 검사 규칙으로 되돌려 같은 실수가 다음부터 자동으로 막히게 만드는 Closed-loop** 구조로 설계했습니다.
...
2. **분류** — 발견된 사례를 처리 방식별로 4갈래로 나눔: ⓐ 코드 패턴으로 자동 검출 가능 ⓑ 변경은 감지되지만 정확한 값은 사람만 앎 ⓒ 코드만으론 판단 불가(외부 시스템 참조 필요) ⓓ 잘못 잡은 것(오탐).
3. **변환** — 위 분류에 따라 정책(데이터로 표현 가능) / 규칙(사람 판단 근거) / 검사 스크립트(자동 검출) / 사람 확인 단계 중 어디로 옮길지 결정.
...
- **스케줄러 안전장치 누락** — 예약 실행 코드(`@Scheduled`) 42개 중 11개만 보호 블록을 갖춰 나머지에서 실패가 조용히 묻히던 상태 → 어노테이션 위치와 본문 보호 블록 유무를 자동 판정하는 검사로 변환 → 이후 새 예약 코드는 누락이 자동 검출
- **외부 시스템 불일치** — 내부 코드가 Jenkins 실제 지원 항목과 어긋난 채 운영에 노출 → 처음엔 자동 검사를 넣었으나 "코드만 봐도 이해되는 게 좋은 코드"라는 개발자 피드백으로 자동 검사를 거두고 **"사람에게 먼저 질의"** 규칙으로 전환 → 같은 유형 조사가 3턴 → 1턴으로 단축(대표 사례 기준). *자동화가 항상 정답은 아님*을 보여 준 사례
...
| 계층 | 자산 | 자료 형식 | 수량 |
|------|------|----------|------|
| A | 기록, 보관 (사례, 결정, 점검 이력) | .md/.yaml (append-only) | 사례 108 외 (아래 ※) |
| B | 검증 규칙 (사람이 읽는 의도) | .md | 34종 |
| C | 정책 (데이터 선언) | .yaml | 12종 |
| D | 자동 검사 스크립트 | .py (git hook) | 29개 |
| E | AI 협업 자산 | skill, agent | 59 + 50 |

> ※ Layer A 세부: 누적(고정) — 사례 108, 결정 기록 16 / 시계열(머지마다 누적, 측정 시점에 따라 변동) — 점검 보고서·자산 스냅샷은 머지마다 1건씩 쌓이고, 자기개선 회차는 활성 13(누적 15)

이 구조의 핵심은 **"왜, 무엇을, 어떻게"의 분리**입니다.

![3계층 분리 — 의도 / 데이터 / 실행](AI자동화/13-layer-separation-v2.png)

- **의도(규칙 .md)** 는 왜 막는지를 사람이 읽고 판단합니다.
- **데이터(정책 .yaml)** 는 무엇을 막는지(고객사, 경로, 등급 목록)를 담습니다. 신규 고객사가 들어오면 이 **정책 파일(.yaml)의 고객사 목록에 한 항목만 추가**합니다(검사 스크립트 코드는 수정하지 않습니다).
...
- **컨벤션 자동 점검** — 의심 패턴 11종 중 4종(예: 미완성 표시 주석, 빈 메시지 키 등)은 정규식으로 저장(커밋) 시점에 자동 검출합니다. 나머지 7종은 코드 구조 분석이 필요해 사람 확인 단계로 둡니다.
...
![검증 규칙 변경 위험도 3등급 (자동, 승인, 금지)](AI자동화/46-tier-classification-v2.png)
...
![git hook 한 번 설치 후 자동 실행](AI자동화/47-hook-install-v2.png)
...
검사가 실제 코드를 잡는 방법은 세 가지입니다 — ① **정규식**(특정 텍스트 패턴), ② **괄호 짝 세기**(`@Scheduled` 메서드의 여는 중괄호부터 짝이 맞는 닫는 중괄호까지를 깊이 카운팅으로 추출해, 그 본문 안에 try-catch나 실패 기록 호출이 있는지 확인 — 정규식만으로는 메서드 경계를 못 잡아 본문 추출 단계를 따로 둔 것), ③ **변경 파일만 골라 검사**(git이 알려 주는 바뀐 파일만 보므로 전체 빌드보다 빠름). 이 방식의 장점은 세 가지입니다 — 개발자가 별도 도구를 돌릴 필요 없음, 변경분만 봐서 빠름, 새 검사 추가가 쉬움. (정책과 검사 로직 분리의 이점은 §4.3에서 다룹니다.)
...
- **결과** — 검사 코드(.py)를 건드리지 않고 정책(.yaml)만 추가하므로, 여럿이 공유하는 환경에서도 신규 고객사·규칙 추가가 다른 기여자 작업과 충돌하지 않습니다. (변경 파일이 1~2개로 주는 효과는 §4.3.)

### 5.2 저장(커밋) 시점 고객사 경계 자동 감지


![커밋 시점 3종 자동 검사](AI자동화/53-commit-boundary-v2.png)

- **문제** — 공통 코드에 `if (고객사 == "특정사")` 같은 분기가 섞이면 여러 고객사 모두에 전파됩니다. 비밀값(키, 토큰, 비밀번호)이 코드 이력에 한 번 들어가면 교체 전까지 노출 위험이 남습니다.
- **선택지** — (a) CI(빌드, 테스트 자동화) 단계 검사 (b) 위반 시 자동 수정 (c) 저장 시점 차단 → **채택**.
...
- **결과** — 여러 고객사 × plugin × 모듈 매핑을 정책에 선언해, 저장 직전 경계 침범, 비밀값, 보호 경로 변경을 함께 검사합니다.

### 5.3 변경 영향 미리보기 (작성 전 강제)

- **문제** — 영향 범위를 모른 채 구현으로 직진하면, 한 파일 변경이 전 고객사로 퍼진 뒤에야 문제를 발견합니다.
- **만든 것** — 코드 작성 전에 *① 영향 모듈 ② 영향 고객사 ③ 함께 바뀔 것(테스트, 문서, 외부 연동) ④ 위험 top 3 ⑤ 진행 여부 판단* 다섯 가지를 강제로 정리·평가하게 했습니다. "영향을 특정할 수 없음"은 허용하지 않고 실제로 찾게 합니다.
...
![외부 연동은 추측 말고 사람에게 먼저 질의](AI자동화/55-external-contract-v2.png)
...
- **결과** — 외부 연동 정의가 관여하는 작업은 **코드 추측을 멈추고 사람에게 먼저 묻게** 했습니다. 같은 유형 조사가 3턴(AI와 사람이 주고받은 횟수) → 1턴으로 단축(대표 사례 기준). 답을 못 받으면 결론을 "추정" 상태로 표시하고, 다른 작업의 입력으로 쓰지 않습니다.

---

## 6. 결과

![Before / After — 사람 검토 의존에서 자동 검증으로](AI자동화/before-after-v3.png)
...
- 규칙 34종, 정책 12종, 검사 스크립트 29개, AI 협업 자산(작업 절차서 59개, 작업자 역할 정의 50개) 운영
- "현재 상태"를 매번 다시 재지 않도록, 측정 대상 17종 중 12종에 유효기간을 둬 컨텍스트 비용 폭증 차단
...
공통 코드를 여러 고객사가 공유하고, 다른 부분만 별도 모듈(plugin 4개)과 설정 파일로 분리합니다. 고객사 경계 매핑(여러 고객사 × 모듈)을 정책에 선언해, 저장 시점 검사가 경계 침범을 자동으로 잡습니다.

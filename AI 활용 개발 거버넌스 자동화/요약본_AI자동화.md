...
>
> 수치는 별도 표기가 없으면 **2026-05-29 작업 브랜치에서 코드를 직접 세어 집계한 실측값**입니다.
...
- **사례 1 (스케줄러 안전장치 누락)**: 예약 실행 코드(`@Scheduled` 메서드) 42개 중 11개만 오류 보호 블록(try-catch)을 갖춰, 나머지 31개에서 실패가 조용히 묻히던 것을 발견(실측 grep 26%) → 자동 검사로 만들어, 이후 새 예약 코드는 보호 누락이 자동으로 잡힙니다.

...
**Result**
사람 리뷰 부담이 체감상 30분 → 10분 수준으로 줄었고(대표 PR 기준, 별도 측정 체계 없음), 자동 검증으로 잡힌 위반 75건이 FAILURE_PATTERNS에 누적되어 다음 cycle 학습 데이터로 사용된다.
...
**Result**
대표 사례 기준 PR 변경 파일 수가 18개 → 7개로 줄었고, 리뷰어 검토 시간도 체감상 30분 → 12분 수준으로 단축됐다(별도 측정 체계 없음). preview 통과 후 머지 차단된 PR은 0건이다.

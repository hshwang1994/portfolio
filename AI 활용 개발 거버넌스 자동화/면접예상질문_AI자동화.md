...
> 이 노트는 `문제해결_8선_AI자동화.md`(8가지 문제·해결·검증된 결과)와 `프로젝트개요_AI자동화.md`(전체 배경·정의·결과)를 면접 대화용으로 푼 것이며, **같은 수치·같은 framing**을 씁니다. 질문↔8문제 매핑은 맨 아래 부록 참고.
...
> **(2) 정성, 체감치** — 리뷰 시간 단축, false positive 비율, 토큰 절감률, "N턴→1턴", "머지 차단 N건", **그리고 답변에 나오는 처리 시간 추정(Tier별 5분/30분, 새 고객사 4시간, 정책 추가 5분, hook 작성 1시간 등 분·시간 단위)** 은 별도 측정 시스템이 없어 체감 또는 대표 사례 1건 기준이다. **`문제해결_8선`·`프로젝트개요` 문서는 이런 추정치를 아예 넣지 않았고**(검증된 구조 수치만 사용), 면접에서도 이 (2) 유형은 반드시 "체감상/대표 사례 기준"으로 답할 것 — 출처를 못 대는 정밀 수치는 신뢰를 깎는다.
> 면접에서 **(2) 유형은 반드시 "체감상" 또는 "대표 사례 기준"으로 답하고 정밀 수치를 단정하지 말 것.** 출처를 못 대는 수치는 신뢰를 깎는다.

> **용어 (한 번만 정리)**: 답변에 나오는 `rule NN`은 사내 검증 규칙 문서(`.claude/rules/`)의 번호, `skill`은 AI 도구(Claude Code)에서 자동·수동으로 호출하는 작업 절차, `hook`은 git 단계(커밋·머지 등)마다 자동 실행되는 검사 스크립트, `Tier`는 변경 위험도/문서 계층을 나누는 등급을 가리킨다.
...
**Action**
Bash, Node.js, Python 세 가지를 두고 비교했다. Bash는 Windows Git Bash와 PowerShell에서 동작이 갈렸고, Node는 별도 의존성 설치가 hook 실행 직전에 깨질 위험이 있었다. Python은 표준 yaml, re, subprocess만으로 정책 파싱과 정규식 검사를 모두 처리 가능했다. pre-commit, commit-msg, post-merge, post-commit, post-rewrite, pre-push, pre-merge-commit 7종 git hook 단계의 29개 파일을 모두 Python으로 통일했고, install-git-hooks.sh로 한 번에 등록했다.
...
**Result**
정적 정책 12개 + 동적 hook 29개의 이중 구조로, 정책 수정 시 코드 재배포가 불필요한 운영을 달성했다. (false positive 비율은 별도 측정 체계가 없어 정밀 수치로 단정하지 않고, 오탐이 나오면 즉시 규칙을 보정하는 방식으로 관리한다.)
...
**Action**
11종을 정규식 검출 가능 여부로 분류했다. TODO 마커, 빈 i18n 키 getMessage(""), hasLength vs hasText 혼동, printStackTrace 4종은 단일 토큰 매칭으로 처리 가능해 pre_commit_suspicious_pattern_check.py에 정규식으로 정의했다. 나머지 7종 (null 체크, stream 재사용, regex lookaround dead code, Mapper case 의존, NumberFormat 래핑, self-reference, Collection 변환)은 AST 파싱 또는 의도 추론이 필요해 정규식만으로는 false positive 위험이 컸다. 이 7종은 write-quality-tdd skill에서 사람 + AI 가 함께 검토하는 수동 게이트로 남겼다.

**Result**
자동화 4종은 정규식 기반이라 도구가 일관되게 처리하고, 수동 7종은 PR 리뷰에서 사람이 판단하도록 분리해 도구 책임과 사람 책임의 경계가 명확해졌다. (발견 건수 같은 정량 수치는 별도 집계 체계가 없어 단정하지 않는다.)
...
**Result**
와이어프레임 합의가 빨라졌고(대표 사례 기준 약 3분), 이모지 대신 ASCII 태그([PASS]/[FAIL] 등)를 도입해 폰트, 플랫폼이 달라도 표 정렬이 깨지지 않는 일관된 가독성을 확보했다.
...
**Action**
local hook은 advisory로 두고 commit 차단 없이 reminder만 출력했다. Bitbucket Pipeline에는 하네스 일관성 검증 7종(verify_harness_consistency, verify_harness_surface, verify_skill_sync, check_doc_links, validate_claude_structure 등)을 step으로 등록해서, 위반이 있으면 파이프라인이 실패(red)하도록 했다. 단 고객사 경계 검사(verify_customer_boundary.py)는 CI가 아니라 로컬 pre-commit advisory로 동작한다(경계 위반은 코드만으로 강제 차단하기보다 사람이 판단하도록 경고만 출력). 로컬에서 놓친 하네스 일관성 위반은 PR 시 CI 빨간불로 잡힌다.

**Result**
local advisory(빠른 피드백) + CI 검증(하네스 일관성 7종, 실패 시 파이프라인 red) 이중 구조로 commit 단계 마찰을 최소화했다. (검출률 같은 정량 수치는 별도 측정 체계가 없어 단정하지 않는다.)
...
**Action**
"되돌릴 수 있는가"와 "위반 후 비용"을 기준으로 분기했다. post-merge나 post-commit처럼 이미 일어난 사건은 hard block이 무의미해서 advisory로만 출력했다. pre-commit에서 보호 경로 변경, Tier 3 권한 변경, 시크릿 하드코딩은 hard block으로 막았다. 의심 패턴 11종은 false positive 위험 때문에 advisory로 두되 reminder를 강화했다. CI Pipeline에서는 하네스 일관성 검증 7종(verify_harness_consistency 등)이 실패 시 파이프라인을 red로 만들어 게이트 역할을 한다(고객사 경계 검사는 CI가 아니라 로컬 pre-commit advisory).

**Result**
강제 차단은 비밀값·보호 경로·정책 위반 등 소수에만 걸고 나머지 대부분은 advisory(경고)로 운영한다(로컬 hook 기준 강제 차단 약 6종 / advisory 약 26종). 개발자 작업 마찰은 commit당 체감 2초 미만이고, 로컬에서 놓친 하네스 일관성 위반은 CI 검증에서 다시 걸린다.
...
**Result**
cycle 처리 시간이 초기보다 짧아졌고(체감 기준, 정량 측정 체계 없음), 미완결 cycle 0건, FAILURE_PATTERNS 75건이 여러 cycle에 걸쳐 모두 cycle 입력으로 흡수됐다.
...
**Action**
customer-boundary-map.yaml에 고객사별 경계 경로를 선언했다. plugin-A, plugin-C, plugin-example, plugin-sdk 4개 모듈과 resources/custom/{profile}/, actions/{profile}/, billing/{profile}/ 디렉터리를 명시했다. pre-commit hook verify_customer_boundary.py가 staged 파일을 검사해서 core 모듈에 고객사 이름 직접 분기가 들어가면 advisory(경고)로 출력한다(이 검사는 로컬 전용이며, 코드만으로 강제 차단하기보다 사람이 판단하도록 경고만 낸다). 브랜치 패턴도 고객사B-*, 고객사A-* 등으로 자동 감지해서 yaml.profile을 매핑했다.
...
**Task**
새 고객사 추가가 기존 고객사들에 영향을 주지 않도록 검증 체크리스트를 만들어야 했다.
...
**Action**
task-impact-preview skill로 영향 범위를 분석했다. 고객사 B는 전용 plugin 모듈이 없고 yaml.profile(고객사B) + resources/custom/고객사B/ 설정으로 분기하는 구조다. core 모듈은 전 고객사 공통 컴파일 대상이라, core enum에 값을 추가하면 (1) plugin이 있는 고객사 A·고객사 C 빌드가 같은 core를 참조해 동시 재빌드·노출되고, (2) 이 enum은 Jenkins 파이프라인 target_type 계약과도 동기화돼야 해 외부 계약 drift까지 유발한다는 결론이 나왔다. 사용자에게 WHY+WHAT+IMPACT 포맷으로 보고하고 두 가지 대안을 제시했다 — (1) 값이 고객사 특화면 core enum 대신 yaml.profile 토글/설정으로 분리, (2) plugin이 있는 고객사(고객사 A/고객사 C)라면 해당 plugin이 plugin-sdk 확장 포인트로 처리. 최종적으로 core enum은 건드리지 않고 profile 설정 레벨로 분리했다.

**Result**
core 변경 0줄, 고객사 특화 분기를 profile 설정으로 격리, 다른 고객사 빌드 영향 0건. 이 사례로 "core enum에 고객사 전용 값 추가 금지" 판단이 customer-boundary 검사 기준에 반영됐다.
...
**Action**
AI는 한 번에 많은 파일을 수정하고 추정과 실측을 섞기 쉬워서 사람 리뷰가 따라잡지 못했다. 그래서 자동 검증을 사람 리뷰 앞 단계로 두었다. pre-commit hook 5종이 의심 패턴, 보호 경로, 커밋 메시지, 의존성 변경, 회귀 영역을 자동 검사한다. post-merge hook은 들어온 코드의 위반 패턴을 검사해서(검사 함수 8개 중 1개는 폐기된 규칙이라 비활성, 실효 7종) docs/ai/incoming-review/ 에 보고서를 자동 생성한다. 사람 리뷰는 AI가 못 잡는 의도 판단과 비즈니스 로직 검증에 집중하도록 분업했다.

**Result**
사람 리뷰 부담이 체감상 줄었고(대표 PR 기준, 정량 측정 체계 없음), 누적된 실패·불일치 사례 75건이 FAILURE_PATTERNS에 기록되어 다음 cycle 학습 데이터로 사용된다.
...
**Result**
대표 사례에서 PR 변경 파일 수와 리뷰어 검토 시간이 체감상 줄었다(정량 측정 체계 없음). preview 통과 후 머지 차단된 PR은 0건이다.
...
**Result**
자동 4종 + 수동 7종의 이중 구조를 한계로 인정하고, 수동 7종에서 반복 발견되는 패턴을 자동화 우선순위 후보로 축적 중이다(발견 빈도는 별도 집계 체계 없이 대표 사례 기준).
...
**Action**
"되돌리기 비용 + 위반 후 비용"이 둘 다 높은 경우만 hard block으로 분류했다. 보호 경로 변경, 시크릿 하드코딩, Tier 3 권한 완화 3개만 block이고 나머지는 advisory다. advisory hook은 reminder를 강하게 출력하지만 commit 차단은 안 한다. 대신 하네스 일관성 위반은 CI Pipeline의 검증 7종(verify_harness_consistency 등)에서 파이프라인 실패로 다시 걸리도록 이중화했다. SKIP env는 의도적 우회를 추적 가능하게 두되 사용 시 commit message에 사유 기록 의무를 rule 90에 추가했다.

**Result**
SKIP env 사용 빈도가 cycle 7 약 30%에서 cycle 15 약 5%로 줄었고(체감 기준), hard block 대상(보호 경로, 시크릿, Tier 3)은 SKIP 경로 자체가 없어 우회가 불가능하다. 작업 마찰은 commit 추가 시간 2초 미만 수준이다.
...
**Action**
초기에는 신뢰도 측정 없이 자동 수정을 시도했다가 import 정렬 hook이 코드 한 줄을 잘못 지우는 사례가 발생했다. 그래서 rule 97 R4에 "자동 수정 금지 - 명시 사용자 검토 필수"를 명시했다. 자동 수정은 import 정렬, 공백 정규화 같이 코드 의미를 바꾸지 않는 좁은 영역만 허용 후보로 두었다. 초기 설계 문서에 "신뢰도 0.95 이상만 자동 수정"이라는 임계값을 적기는 했지만, 실제로 그 정확도를 측정할 체계는 없었다. 그래서 "근거 없는 숫자에 기대 자동 수정을 강행하지 않는다"고 판단하고, 자동 수정 자체를 보류한 채 advisory(경고)로 남겼다 — 잘못된 자동 수정의 회귀 비용이 사람이 직접 고치는 비용보다 크기 때문이다.
...
**Result**
매 작업 전수 측정 대신 캐시를 재사용해 세션 토큰 소비를 줄였고(절감률은 별도 측정 체계가 없어 체감 기준 — 정량 수치로 단정하지 않음), stale 캐시로 인한 잘못된 코드 생성 0건, post-merge hook이 자동 무효화 trigger로 작동해서 사용자 개입 없이 카탈로그 갱신 가능하다.
...
**Action**
1순위는 의심 패턴 7종 중 일부를 PMD/SpotBugs로 자동화하는 것이다. 빌드 시간 증가 비용은 있지만, 수동 게이트에서 반복 발견되는 패턴을 자동 검출로 옮기면 사람 리뷰 시간이 줄어든다. 2순위는 외부 시스템 계약을 EXTERNAL_CONTRACTS.md 카탈로그로 데이터화하는 것이다. Jenkins, VMware, GitLab 등 외부 enum의 실제 지원 집합과 마지막 동기화 일자를 카탈로그에 두면 rule 96 R2 질의 빈도가 감소한다. 3순위는 cycle 자동 트리거다. 현재는 사용자가 cycle 시작을 명시해야 하는데, FAILURE_PATTERNS 누적 임계치 도달 시 자동 cycle 진입 제안하는 hook을 추가하는 것이다.

**Result**
3개 항목 모두 docs/ai/roadmap에 cycle 16~18 후보로 등록했다. 효과 측정 체계가 아직 없어 정량 목표(N% 단축)는 잡지 않았고, 우선순위 근거는 "cycle당 반복 발견 빈도"와 "빌드 시간 비용" 사이의 트레이드오프다.

---

## 부록 — '문제 해결 8선' ↔ 질문 매핑

이 답변들은 별도 문서 `문제해결_8선_AI자동화.md`의 **8가지 문제**를 풀어가는 과정입니다. 각 질문이 어느 문제와 연결되는지 정리했습니다(한 질문이 여러 문제에 걸치면 주(主) 문제에 배치). 면접에서는 어떤 질문이 와도 "이건 8가지 중 N번 문제였습니다"로 연결하면 일관된 서사가 됩니다.

**문제 1. 발견된 오류를 재발 방지 자산으로** — Q6(6단계 거버넌스), Q10(cycle 15회 학습), Q19(Gson 의존성 실패 사례), Q26(정책 YAML vs 코드 hardcoding)

**문제 2. 토큰 비용과 오래된 정보 동시 절감** — Q25(컨텍스트 비용 vs 검증 깊이)

**문제 3. 커밋 전 위험 변경 차단** — Q5(CI 통합), Q9·Q23(advisory vs 강제 차단), Q11(고객사 경계 자동 감지), Q12(새 고객사 추가 비용), Q13(core↔plugin 경계 위반 사례)

**문제 4. 머지 후 위험 자동 보고** — Q15(AI 코드 검증을 왜 만들었나), Q17(review burden 축소)

**문제 5. 외부 시스템 계약 사람 확인 게이트** — Q16(AI 환각 대응), Q21(rule 96 R1 폐기 — 자동 검사를 사람 게이트로)

**문제 6. 검사(hook) 무한 증가 통제** — Q3(11종 중 4종만 자동화), Q20(7종 자동화 불가 인정), Q24(자동 수정 보류)

**문제 7. 검증 시스템 자체 보호 (control plane)** — Q7(Tier 분류 기준), Q8(control plane 분리)

**문제 8. 멀티에이전트 환각 감소** — Q18(Claude Code 한계 · Agent 토큰 초과)

**공통 / 기술 기반 / 본인 기여 / 한계** (특정 문제에 안 묶이고 면접관 추가 질문 대응용) — Q1(Python 선택), Q2(YAML 정책 엔진 한계), Q4(ASCII art 우선), Q14(plugin 모듈 4개 차이), Q22(다시 한다면), Q27(82.7% 단독 기여), Q28(팀·본인 책임 경계), Q29(타사 적용 가능성), Q30(다음 단계 3개)

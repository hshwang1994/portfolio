...
6개 고객사가 한 코드베이스를 함께 쓰는 환경에서, 인력을 충원하는 대신 AI 도구(Claude Code 등)로 개발 생산성을 높이는 방향을 골랐습니다. 다만 단일 코드베이스에 AI를 무작정 도입하면 한 고객사용 변경이 6개 고객사 전체로 사고를 퍼뜨릴 수 있어, **단순히 검사만 하는 하네스가 아니라 — 발견된 오류를 검증 자산으로 되돌려 같은 실수가 다음부터 자동으로 막히게 만드는 닫힌 루프(closed-loop) 자기개선 거버넌스**를 직접 만들었습니다. AI가 만든 결과를 개발 단계, 커밋 단계, 머지 단계에서 자동으로 검사하고, 그 결과를 다시 규칙, 정책, git hook으로 환원합니다.

### 30초 요약

| 구분 | 내용 |
|------|------|
| **포지션** | 개발 생산성과 플랫폼 엔지니어링 성격의 사내 도구. Java/Spring 백엔드 업무를 기반으로 자동화 영역을 단독으로 설계하고 구현 |
| **문제** | 6개 고객사 단일 코드베이스에서 AI가 만든 변경이 고객사 경계, 외부 시스템 계약, 컨벤션을 깨면 5개 고객사 배포로 전파 |
| **핵심 설계** | 검증 의도(규칙 MD 34), 데이터(정책 YAML 12), 실행(git hook Python 29)을 세 계층으로 분리. 6단계 거버넌스를 두고, 실패를 자산으로 되돌리는 폐쇄 루프를 운영 |
| **대표 결과** | 누적 108건 사례를 자동 검출 자산으로 환원. 고객사 경계 위반은 커밋 시점에 자동 차단, 머지 후에는 8종(7 활성) 자동 점검으로 보고 |
| **기여** | 자동화 영역(`.claude/`, `scripts/ai/`) 커밋의 82.7%를 단독으로 작성하고 운영 (2026-05-29 기준) |
| **기술** | Python(표준 라이브러리), Git Hook, YAML, Bitbucket Pipeline |

> 측정 수치는 별도 표기가 없으면 **2026-05-29, 작업 브랜치 `sk-hynix-cmdb-ipband-resource` 기준 실측값**입니다. 시계열로 늘어나는 항목(머지 점검 보고서, 스냅샷, cycle)은 측정 시점에 따라 증가합니다.
...
ClovirONE은 OpenStack, VMware, Kubernetes, Ansible을 통합 관리하는 프라이빗 클라우드 플랫폼입니다. 같은 코드베이스를 6개 고객사(POSCO, SK하이닉스, Smilegate, KISA, INA, ClovirONE)가 함께 씁니다.
...
![추진 배경: 인력은 수평, 요구사항은 가파르게, 생산성은 AI로 끌어올림](AI자동화/02-productivity-gap.png)
...
문제는 ClovirONE이 **프라이빗 클라우드 제품**이라는 점에서 시작됩니다. 고객사 도메인이 달라질 때마다 **자원을 사용하려는 목적, 빌링 구조, 외부 시스템 연동 기준**이 달라집니다. 그런데 이 6개 고객사가 **하나의 코드베이스**를 함께 바라봅니다.

이런 환경에서 AI가 만든 변경을 무작정 반영하면 곧바로 사고로 이어집니다.

- 한 고객사용 분기가 공통 영역에 섞이면 다른 5개 고객사 배포로 그대로 전파됩니다.
...
즉 생산성을 올리려고 도입한 AI가, 통제 장치 없이는 오히려 6개 고객사 전체로 사고를 퍼뜨리는 통로가 됩니다. 그래서 **AI를 위한 거버넌스 체계**가 먼저 필요했습니다.

### 그래서 — 단순 하네스가 아니라 "닫힌 루프(closed-loop) 자기개선 거버넌스"

이 프로젝트는 단순히 AI에게 규칙을 알려주는 설정 묶음(하네스)이 아닙니다. **발견된 오류를 다시 검증 자산으로 되돌려, 같은 실수가 다음부터 자동으로 막히게 만드는 닫힌 루프(closed-loop) 구조의 자기개선 거버넌스**입니다.
...
이 "사고 → 기록 → 자산화 → 자동 차단 → 다시 관찰"의 닫힌 루프가 이 시스템을 단순 하네스와 구분 짓는 핵심입니다. (이 닫는 과정은 §4 동작, 진화 흐름에서 자세히 다룹니다.)

### 왜 기존 도구로는 안 됐나

SonarQube와 Checkstyle 같은 정적 분석 도구는 일반 코드 품질 검사(null 체크, 복잡도, 스타일)는 잘 합니다. 하지만 ClovirONE 고유의 검증 영역은 다룰 수 없었습니다.
...
자동 검증 시스템의 **구조를 설계하고 핵심 부분을 직접 구현**했습니다. 자동화 영역(`.claude/`, `scripts/ai/`) 커밋의 **82.7%**를 직접 작성하고 운영했습니다 (2026-05-29 작업 브랜치 기준 91 / 110 커밋. 같은 영역 기여자 6명 — 본인 외 5명).
...
- 자기개선 cycle 운영 (cycle-001 ~ cycle-015 누적 15회. cycle-001과 002는 archive, cycle-003 ~ cycle-015 활성 로그 13건을 시계열 정량 추적)
- 측정 lifecycle 정책 설계 (측정 대상 17종 설계 중 12종을 `measurement-targets.yaml`에 TTL과 무효화 trigger로 선언, 나머지 5종은 신설 예정)
- 6단계 거버넌스 파이프라인 정립 (observer → architect → reviewer → governor → updater → verifier)
...
아키텍처를 여덟 가지 관점으로 나눠 설명합니다. 같은 시스템을 다른 각도에서 본 것이며, 기승전결 흐름으로 진행합니다.

| 단계 | 관점 | 답하는 질문 |
|------|------|-------------|
| **起 (진입)** | **4.8 워크플로우 호출** | 사용자가 AI를 어떻게 호출하고, AI는 자산 109개(agent 50 + skill 59)에서 무엇을 골라 쓰나? |
| 起 (진입) | **4.3 정적 구조** | 무엇으로 만들어져 있나? 어떤 파일이 어떻게 연결되나? |
| **承 (전개)** | **4.5 개발 흐름** | AI가 코드를 만들어가는 과정은 어떻게 통제되나? |
| 承 (전개) | **4.4 사용 흐름** | 사람과 AI가 이 시스템을 어떻게 쓰나? |
| **轉 (검출)** | **4.1 동작 흐름** | 한 코드 변경이 커밋과 머지 단계에서 어떻게 검증되나? |
| 轉 (검출) | **4.7 자동 실행 메커니즘** | hook 29개가 어떻게 자동 트리거되고 코드를 검출하나? |
| **結 (환원)** | **4.2 진화 흐름** | 한 번 발견된 오류가 어떻게 카탈로그에 저장되고 다음에 자동으로 잡히게 되나? |
| 結 (환원) | **4.6 거버넌스 흐름** | 자동 허용, 사람 승인, 절대 자동 금지를 어떻게 분기하나? |

번호는 작업 순서대로 매겨졌지만, 위 표의 기승전결 순서로 읽으면 시스템의 전체 라이프사이클이 한 흐름으로 보입니다. 이 절에서는 번호 순서(4.1 → 4.7 → 4.8)로 설명합니다.
...
![4.1 동작 흐름](AI자동화/41-execution-flow.png)
...
![4.2 진화 루프 — 한 번 겪은 오류는 다음부터 자동으로 막힌다](AI자동화/42-evolution-loop.png)
...
| 변환 | 처음에는 enum origin 주석을 자동 검사하는 검사 c 슬롯(`post_merge_incoming_review.py`)을 넣었습니다. 그러나 "코드만 봐도 이해 가능한 게 좋은 코드"라는 개발자 피드백으로 origin 주석 의무(rule 96 R1)를 폐기하면서 검사 c를 **비활성화**했습니다. 대신 외부 계약 확인은 규칙(rule 96 R2/R3/R4)으로 옮겨, **코드 작성 전 사용자에게 외부 계약을 먼저 질의**하는 사람 게이트로 의무화했습니다 |
...
이 두 사례가 "수집 → 분류 → 변환 → 검증" 4단계가 실제로 작동한 증거입니다. 누적 108건(FAILURE_PATTERNS 75 + CONVENTION_DRIFT 33)이 모두 이 절차를 거쳐 정책 YAML 12종 / 검증 규칙 34종 / git hook 29개 중 한 곳에 옮겨졌습니다.
...
![4.2 측정 lifecycle](AI자동화/42b-measurement-lifecycle.png)
...
![4.3 검증 자산 5계층 구조](AI자동화/43-layered-architecture.png)

| 계층 | 자산 | 자료 형식 | 수량 |
|------|------|----------|------|
| Layer A | 기록, 보관 체계 | .md/.yaml (append-only, 시계열) | 사례 108 + ADR 16 + incoming-review 39 + snapshots 28 + cycle 13 + 추적, 인수인계 |
...
![4.4 사용 흐름](AI자동화/44-usage-flow.png)
...
![4.5 개발 단계 3중 게이트](AI자동화/45-development-gates.png)

**이 흐름의 핵심 세 가지:**

- **AI가 임의 판단으로 게이트를 건너뛰지 못하도록 정책 레벨에서 reminder를 자동 주입합니다.** "이 정도면 안전하다"는 임의 판단을 막는 advisory 단계입니다. 사용자가 명시적으로 "skip"을 말한 경우에만 reminder를 제외합니다(추후 hard block 전환은 별도 cycle).
- **회사 코드 컨벤션 11종 중 4종은 자동 스캔합니다.** TODO 마커, 빈 i18n 메시지 키, hasLength/hasText 혼동, printStackTrace 등 정규식으로 잡을 수 있는 패턴은 작성 단계에서 자동 검출합니다. 나머지 7종(Null 체크, Stream 재사용 등)은 AST 분석이 필요해 사람 확인 게이트로 분리했습니다.
...
### 4.6 거버넌스 흐름 — 3중 Tier 체계

검증 시스템 자체에도 변경이 발생합니다. 정책 YAML 한 줄 갱신, 새 hook 추가, 보호 경로 변경은 모두 다른 위험도를 가집니다. 매번 사람이 판단하기보다 **3중 Tier 체계**로 분류해 자동/수동 분기를 정해 두었습니다.
...
![4.6 변경 등급 Tier 1/2/3](AI자동화/46-tier-classification.png)
...
- **자동과 수동의 경계를 정책 파일로 선언**합니다. 매번 사람이 판단하지 않고 `auto-fix-patterns.yaml`이 자동으로 분기시킵니다.
- **Tier 3은 자동 변경 절대 금지**입니다. 보호 경로 제거 같은 위험 변경은 사용자 명시 승인 없이는 어떤 자동화도 시도하지 않습니다.
- **Tier B의 자동 변경도 advisory 보고 필수**입니다. 자동 변경했더라도 사람이 다음 작업 전에 보고서로 확인할 수 있습니다.

이 3중 Tier 체계 덕분에 "자동화할 것은 안전하게 자동화하고, 사람 판단이 필요한 것은 명확히 묻고, 절대 자동화하면 안 되는 것은 차단"이 한 시스템 안에서 분리 운영됩니다.

#### 6단계 거버넌스 파이프라인

Tier 2(사람 승인 필요) 변경은 임의로 통과시키지 않고 6단계 파이프라인을 거칩니다. 각 단계는 별도 agent로 구현되어 있으며, **이전 단계와 같은 인스턴스가 다음 단계를 겸할 수 없도록 정책으로 분리합니다**(Author ≠ Reviewer 정책 — 프롬프트 기반 분리이며 OS 수준 기술 강제는 아님).

![4.6 6단계 거버넌스 파이프라인](AI자동화/46c-governance-pipeline.png)
...
git이 특정 시점(commit, merge, push 등)에 자동으로 실행하는 스크립트입니다. 저장소의 `.git/hooks/` 디렉터리 안에 있는 실행 파일이 트리거됩니다. bash 스크립트도 가능하고, Python을 호출하는 wrapper도 가능합니다.

#### 설치 흐름

![4.7 git hook 설치와 트리거](AI자동화/47-hook-install.png)
...
post-merge wrapper도 같은 패턴입니다. 4개 Python hook을 모두 advisory(`|| true`)로 호출해 머지 후에는 차단 없이 보고서만 생성합니다.
...
**개발자가 처음 한 번만 `install-git-hooks.sh`를 실행**하면, 이후 모든 git commit, merge, push가 위 wrapper를 거치므로 29개 Python hook이 적절한 시점에 자동 실행됩니다. 신규 hook을 추가해도 `install-git-hooks.sh`를 다시 실행만 하면 wrapper가 갱신됩니다.
...
![4.8 AI 협업 워크플로우](AI자동화/48-ai-workflow.png)
...
| 현재 고객사 컨텍스트 | git branch 파싱 (`sk-hynix-*`, `posco-*` 등) | 고객사별 Spring profile, 컨벤션, 보호 영역 |
...
(a)는 신규 고객사가 들어올 때마다 검증 코드를 수정해야 해서 모든 기여자 환경에 영향을 줍니다. (b)는 SonarQube custom rule이 ClovirONE 고유 정책(고객사 경계, 외부 계약)을 표현하기 어렵고, 정책 갱신 시 도구를 다시 빌드해야 합니다. (c) 3계층 분리는 YAML 한 블록 추가만으로 즉시 적용되고 검증 코드 자체는 건드리지 않아 다른 기여자 환경에 영향이 없습니다.

**4) 내가 만든 기준 / 구조**

검증의 의도는 규칙 문서(.md)에, 데이터는 정책 파일(.yaml)에, 실행은 git hook(.py)에 분리했습니다.

![5.1 검증 의도와 데이터의 분리 (3계층)](AI자동화/51-layer-separation.png)
...
코드를 한 줄이라도 읽거나 쓰는 작업이면 의심 패턴 11종(null 누락, Stream 재사용, 빈 i18n 메시지 키, hasLength/hasText 혼동, TODO 마커 등)도 함께 자동 스캔합니다.

큰 단위 작업이 하위 작업으로 쪼개지면 각 하위 작업 시작 시점마다 다시 실행해, 처음에는 보이지 않던 추가 범위를 일찍 감지합니다.

**5) 어느 시점에서 자동으로 검증되는가**

코드 작성 단계 진입 직전. 사용자가 "그냥 진행해줘" / "프리뷰 skip"을 명시한 경우에만 건너뜁니다(AI 자체 판단으로는 생략 금지).

**6) 실패하면 어떻게 처리되는가**

- LOW: 바로 구현 진입
- MED: 선택지 비교 단계 추가
- HIGH: 영향 브리핑 문서 작성 + 사용자 명시 승인 대기
- 5섹션 중 하나라도 산출 못 하면 게이트 통과 불가 ("영향 모듈 불명"은 허용 답변 아님)

**7) 결과적으로 무엇이 줄었는가**

영향 범위를 모르는 채 구현으로 직진하는 패턴이 차단되었고, AI가 "이 정도면 안전하다"고 임의 판단하던 케이스가 게이트에서 막힙니다.

---

### 5.3 커밋 단계 고객사 경계 자동 차단

**1) 어떤 문제가 있었는가**

공통 코드에 `if (profile == "skHynix")` 같은 고객사 분기가 섞이면 6개 고객사 모두에 전파되어, 다른 고객사 배포에서 의도치 않은 동작을 일으켰습니다. PR 후반에서야 다른 고객사 담당자가 발견하면 이미 검증 비용이 크게 쌓여 있는 상태였습니다.
...
6개 고객사 × 3개 plugin × 15개 모듈의 매핑을 정책 파일에 선언하고, 커밋 직전 hook이 변경 파일이 경계를 침범하는지 검사합니다. 같은 hook이 시크릿 노출과 보호 경로 변경도 함께 검사합니다.

![5.3 커밋 단계 3종 병행 검사](AI자동화/53-commit-boundary.png)
...
`git merge`, `git pull`, `git rebase` 직후 자동 실행. 사람이 다음 작업을 시작하기 전.

**6) 실패하면 어떻게 처리되는가**

- 위반 없음: "이상 없음" 보고서로 종료
- LOW/MED 위반: 보고서에 기록만 (정보 제공)
- HIGH 위반: 보고서 + 후속 작업 티켓으로 자동 연결 (정리 책임 명시)
- 차단은 하지 않음 — 이미 일어난 머지를 되돌릴 수 없으므로

**7) 결과적으로 무엇이 줄었는가**

머지 후 8종 위반이 사람 검토 전에 보고서로 자동 노출됩니다. 정리 책임을 후속 티켓으로 명시하므로 위반이 보고서에만 남아 잊혀지는 일이 줄었습니다.
...
![5.5 외부 시스템 계약 사람 확인 게이트](AI자동화/55-external-contract.png)
...
| 결정 | 근거 |
|------|------|
| 기존 정적 분석 도구를 대체하지 않고 보완 구조로 설계 | SonarQube와 Checkstyle은 일반 코드 품질 검사는 가능하지만 ClovirONE 고유의 영역(고객사 plugin 경계, 외부 시스템 계약, 운영 컨벤션, AI 작업 흐름)까지는 표현 어려움. 프로젝트 특화 검증을 직접 설계. |
...
| 검증 시스템 변경은 6단계 파이프라인 + Author ≠ Reviewer 강제 | 같은 인스턴스가 작성과 검증을 겸하면 사각지대 발생. observer → architect → reviewer → governor → updater → verifier 6단계로 분리, 각 단계 별도 agent. |
...
- **AI 도구 의존**: skill의 자동 호출은 Claude Code 같은 AI 도구에 의존합니다. 도구가 없으면 사용자가 슬래시 커맨드로 수동 호출해야 합니다.
- **advisory 위주**: pre-commit hook 9종 중 일부만 strict block(시크릿, 보호 경로 등). 의심 패턴 검출은 대부분 advisory라 사용자가 무시할 수 있습니다.
- **Java AST 분석 한계**: 11종 의심 패턴 중 4종만 정규식으로 자동 검출 가능. 나머지 7종(Null 체크, Stream 재사용, Regex lookaround 등)은 AST 분석이 필요해 사람 게이트로 분리되어 있습니다.
- **운영 측정 데이터 부족**: cycle log로 검증 시스템 자체의 변화는 시계열 추적되지만, AI 코드 생성 비중 변화나 사람 리뷰 시간 단축 같은 ROI는 별도 측정 데이터가 없습니다.
- **고객사 1곳 적용**: 현재 ClovirONE 1개 저장소에 적용 중. 다른 회사나 다른 저장소에 일반화한 사례는 아직 없습니다.
- **Hook 설치 의존성**: 신규 기여자가 `install-git-hooks.sh`를 한 번이라도 실행하지 않으면 29개 hook이 동작하지 않습니다. 강제 설치 메커니즘(Gradle task 등) 통합이 필요합니다.
- **Windows 환경 호환성**: wrapper는 bash 기반이라 Windows 개발자가 git-bash를 쓰지 않는 환경에서는 hook 트리거가 불완전할 수 있습니다.
...
| hook 오탐, 룰, 문서, 브랜치, 고객사 등 | 13 | hook-false-positive 2, rule-update 2, branch-gap 2, cross-customer 1, scheduler-recorder-misuse 1 외 단발 | 공통 변경 시 POSCO 0원 회귀, advisory인데 commit 차단한 오탐 |
...
| 테마 | 대표 DRIFT |
|------|-----------|
| 모듈, 구조 정리 | redfish 모듈 누락(001), clovirone-server 빈 모듈(007), 참조 디렉터리 미존재(003) |
...
- 고객사: 6곳 (POSCO, SK하이닉스, Smilegate, KISA, INA, ClovirONE)
- plugin 모듈: 4개 (plugin-sdk 1 + 고객사별 3: plugin-posco, plugin-smilegate, plugin-example)
- Gradle 모듈: 15개
...
| `customer-boundary-map.yaml` | 한 고객사용 분기(예: SK하이닉스 전용 로직)가 공통 코드에 섞여 다른 고객사 배포로 그대로 전파된 사례가 반복됨 |

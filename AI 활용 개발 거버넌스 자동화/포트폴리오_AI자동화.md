...
| **핵심 설계** | 검증을 세 계층으로 분리:<br/>· 규칙 문서(왜 막나, MD 34종)<br/>· 정책 파일(무엇 기준, YAML 12종)<br/>· 자동 실행 스크립트(어떻게 막나, Python hook 29개)<br/>+ 발견된 오류를 되돌리는 **Closed-loop**를 6단계 거버넌스로 운영 |
| **대표 결과** | 누적 108건 사례를 자동 검출 자산으로 환원<br/>· 비밀값·보호 경로 침범 → 커밋 시점 **차단**<br/>· 고객사 경계 위반 → 자동 **감지(경고)**<br/>· 머지 후 → 8종(7 활성) 자동 점검 **보고** |
...
본문 설명은 절 번호 순서(4.1 → 4.8)로 진행합니다. (위 표는 라이프사이클 순서로 정렬했으니, 흐름으로 이해하고 싶으면 표 순서대로 따라 읽어도 됩니다.)
...
- 일부 검사 스크립트는 과거 위반/정상 케이스를 내장한 자가 테스트(`--self-test`, 통합 실행은 `run_harness_self_test.py`)를 갖춰, 정책을 바꿀 때 회귀를 즉시 확인
...
- **수집** — 운영 점검 중 `@Scheduled` 메서드 42개 중 11개만 try-catch로 보호됨을 확인. 나머지는 실패가 조용히 묻히는 위험(silent fail) → FAILURE_PATTERNS.md에 "스케줄러 silent fail"로 기록
- **분류** — "자동 검출 가능": `@Scheduled` 어노테이션 위치 + 본문 try 블록 유무를 정규식 + 중괄호 매칭으로 판정
- **변환** — 규칙 1개(rule 92 R8 "try-catch 의무") + git hook 2개: `pre_commit_scheduler_guard.py`(커밋 advisory) / `post_merge_incoming_review.py`의 검사 a(머지 후 보고)
- **검증** — 기존 42개를 hook에 통과시켜 미보호 31개 검출 확인. 이후 신규 `@Scheduled` 추가 시 자동 검출 작동

**사례 2 — 외부 시스템 enum 불일치 (2026-04-23)**

- **수집** — `CustomActionPhaseType.DAY_1 = [PRE_OS]` enum이 Jenkins 파이프라인 실제 지원 `{redfish, linux, windows, esxi}`와 어긋난 채 운영에 노출. AI가 코드만 보고 가설을 늘려가다 3턴 추론 끝에 사용자가 외부 계약을 알려주면서 발견 → FAILURE_PATTERNS.md에 "external-contract-unverified"로 기록
- **분류** — "부분 가능": enum 변경 사실은 자동 감지되지만, 외부 시스템이 실제로 어떤 값을 지원하는지는 운영자만 앎
- **변환** — 자동 검사 → 사람 게이트로 바꾼 3스텝 전환:
  - (1) 처음엔 enum origin 주석을 자동 검사(`post_merge_incoming_review.py`의 검사 c)로 도입
  - (2) "코드만 봐도 이해 가능한 게 좋은 코드"라는 개발자 피드백 → origin 주석 의무(rule 96 R1) 폐기, 검사 c **비활성화**
  - (3) 외부 계약 확인을 규칙(rule 96 R2/R3/R4)으로 옮겨, **코드 작성 전 사용자에게 먼저 질의**하는 사람 게이트로 의무화
- **검증** — 사람 질의 게이트 전환 후 같은 유형 조사가 3턴 → 1턴으로 단축(대표 사례 1건, 통계적 표본 아님). 자동 origin 검사를 거둔 것은 "자동화가 항상 정답은 아니다"의 사례 — 코드만으로 알 수 없는 영역은 사람에게 맡기는 게 더 빠르고 정확
...
| 계층 | 자산 | 자료 형식 | 수량 |
|------|------|----------|------|
| Layer A | 기록, 보관 체계 | .md/.yaml (append-only) | 사례 108 외 (세부는 아래 "기록 6종" 표) |
...
└── docs/ai/                         운영 문서 + 사례 카탈로그 (Layer A)
    ...
    ├── incoming-review/    (시계열)   머지 후 자동 점검 보고서 (머지마다 누적)
    ├── harness/snapshots/  (시계열)   검증 자산 변동 스냅샷 (머지마다 누적)
    ├── harness/cycle-*-log.md (13개) 자기개선 회차 로그 (활성 13, 누적 15)
...
| 점검 보고서 | incoming-review/*.md | 머지마다 8종 자동 점검 결과 | 머지마다 누적(시계열) |
| 자산 변동 기록 | harness/snapshots/*.yaml | 검증 자산이 언제 무엇이 바뀌었나 | 머지마다 누적(시계열) |
| 자기개선 회차 기록 | harness/cycle-*-log.md | 매 cycle drift 감지, hook 추가, veto 비율 | 13건 |
| 추적, 인수인계 기록 | TRACEABILITY_MATRIX, TEST_HISTORY, handoff/, impact/ | 요구→구현 추적, 테스트 수행, 세션 인수인계 | 카탈로그 + 27건 |
...
| 체계 | 분류 기준 | 등급 | 처리 방식 |
|------|---------|------|----------|
| **문서 계층** | 보존 중요도 | 0 최상위 정본<br/>1 진입 인덱스<br/>2 상세 정책<br/>3 이력 감사 | 어떤 문서를 참조할지 결정 |
| **변경 등급** | 거버넌스 통제 | 1 자동 허용<br/>2 사람 승인 필요<br/>3 절대 자동 금지 | 검증 시스템 변경을 자동/수동/차단으로 분기 |
| **자동 수정 신뢰도** | 자동화 안전성 | A 결정론적(1.0)<br/>B 의도 영향 낮음(0.95+, 설계값)<br/>C 수동 검토 필수 | 자동 수정 후보 분류 (현재 자동 수정은 미운영, §8.2) |
...
- **자동과 수동의 경계를 정책 파일로 선언**합니다. 어떤 검사를 강제 차단/경고/사람 게이트로 둘지를 매번 사람이 판단하지 않고 정책 파일이 분기합니다. (`auto-fix-patterns.yaml`은 "자동 수정 후보" 패턴과 신뢰도 등급을 선언해 두지만, 자동 수정 자체는 현재 운영하지 않습니다 — §8.2 참조.)
- **변경 등급 Tier 3(절대 자동 금지)** 은 자동 변경을 아예 시도하지 않습니다. 보호 경로(수정 금지로 지정한 파일, 디렉터리) 제거 같은 위험 변경은 사용자 명시 승인만 가능합니다.
- **자동 수정 신뢰도 Tier B(의도 영향 낮음)** 로 분류된 패턴조차도, 실제로 자동 수정하지 않고 advisory(경고)로만 보고합니다. 잘못된 자동 수정의 회귀 비용이 커서 운영은 보류 중이며, 사람이 다음 작업 전에 보고서로 확인합니다(§8.2).
...
#### 검증 시스템 자체 보호 — control plane 3중 방어

검증 장치 자체가 잘못 바뀌면 보호 기능이 사라집니다. 보호 경로 삭제, 권한 완화, 고객사 경계 완화 같은 변경은 일반 코드와 다르게 다룹니다.

![검증 시스템 자체 보호 — control plane 3중 방어](AI자동화/57-control-plane-defense-v2.png)

> 보호장치를 푸는 변경은 ① 설계 검토(막던 위험이 다시 풀리나), ② 거버너 승인(변경 사유가 기록됐나), ③ 실행 재확인(사용자 명시 승인인가) 세 관문을 거칩니다. 하나라도 막으면 반영되지 않고, 통과 경로는 사용자 명시 승인뿐입니다.

#### Hook 증가 통제 — 추가 기준과 제거 기준

발견된 문제를 hook으로 만들면 좋아 보이지만, 문제마다 계속 추가하면 커밋 대기 시간이 길어지고 경고가 실제 오류보다 많아집니다. 그러면 개발자가 경고를 확인하지 않고 넘기게 됩니다.

![Hook 증가 통제 — 추가 기준과 제거 기준](AI자동화/56-hook-lifecycle-v2.png)

> 모든 사례를 hook으로 만들지 않습니다. 반복된 유형인지, 자동으로 판정 가능한지, 오탐이 개발을 방해하지 않는지를 먼저 봅니다. 추가한 뒤에도 주기 점검에서 더 이상 안 잡히거나 경고만 쌓이는 hook은 정리(archive) 대상으로 둡니다.
...
위에서 따로 본 세 메커니즘(정규식 → 중괄호 매칭 → git diff)이 실제 커밋 한 번에서 어떻게 한 줄기로 결합되는지, 같은 로직의 end-to-end 흐름으로 추적합니다.
...
- **git 네이티브**: 별도 CI 대기 없이 커밋 즉시 피드백 (정책/로직 분리의 이점은 §4.3 참조)
...
같은 작업을 시간 순서(시퀀스)로 보면, 환각을 막는 지점이 더 분명해집니다.

![멀티에이전트 환각 방지 — 보고가 아니라 실측으로 넘긴다](AI자동화/58-multi-agent-verify-sequence-v1.png)

> 작업자의 "완료" 보고만으로 다음 단계로 가지 않습니다. 메인 세션이 git diff로 무엇이 바뀌었는지, 변경이 지시 범위 안인지, 테스트가 통과했는지를 실측한 뒤에 종합합니다.
...
- 머지 후: 영향 범위 확인은 사람 질의 게이트로 처리(자동 origin 검사 '검사 c'는 폐기·비활성 — 위 4) 참조)
...
- **외부 시스템 연동 오류를 '코드 추측'이 아니라 '사람 선확인'으로 전환** — 대표 사례에서 조사 시행착오를 3턴 → 1턴으로 줄이고, 잘못된 가설로 인한 재작업을 사전에 차단(작은 표본의 대표 사례 기준이라 퍼센트로 일반화하지 않음)
...
| 항목 | Before | After |
|------|--------|-------|
| AI 코드 작성 진입 | "이 정도면 안전"하다며 바로 작성 시작 | 영향 5개 항목 + 컨벤션 확인 후에만 진입 |
| AI 코드 컨벤션 준수 | 사람이 다시 다듬음 (빈 i18n, hasLength/hasText 등) | 의심 패턴 11종 자동 스캔으로 작성 단계 검출 |
| 고객사 경계 위반 발견 시점 | 리뷰 후반에 타 고객사 담당자가 수동 발견 | 커밋 시점 자동 감지, 또는 머지 직후 보고서 |
| 외부 연동 불일치 원인 파악 | 코드만 보고 추측, 3회 시행착오 | 담당자 선확인으로 1회에 원인 파악 |
| 새 고객사 추가 | 검증 코드 직접 수정 | 정책 YAML에 한 블록만 추가 |
| 오류 처리 방식 | 1회성 수정으로 종료 | 정책·규칙·hook으로 환원, 다음부터 자동 검출 |

**단계별 동작** (별도 표기 없으면 "설계상 이런 경로를 막도록 구성"이고, 실제 운영 데이터가 있는 항목만 **[실측]** 으로 표시합니다 — 리뷰 시간 절감 같은 ROI 정량 측정 체계는 §8.1대로 아직 없습니다)

- **[설계상] 개발 단계**: AI가 코드 작성에 진입하기 전에 영향 검토와 컨벤션 준수를 강제. AI가 만든 코드도 사람이 직접 만든 코드와 같은 기준으로 검토
- **[설계상] 커밋 단계**: 보호 경로 변경·시크릿 노출은 강제 차단, 고객사 코드 경계 침범은 advisory(경고)로 감지
- **[설계상] 머지 후 단계**: 같은 유형의 오류 위험, 외부 시스템 연동 항목 불일치, 컨벤션 위반을 자동 보고
- **[실측] 자기개선 운영**: 누적 15회 cycle log로 시계열 추적 (cycle-001 ~ cycle-015. 활성 로그 cycle-003 ~ 015 13건, 초기 2건은 archive). 매 cycle마다 drift 감지 수, 신규 hook 추가, reviewer veto 비율을 정량 기록
- **[설계상] 컨텍스트 비용 관리**: 측정 대상 17종 설계 중 12종에 TTL과 무효화 trigger를 선언해, AI가 매번 전수 측정하지 않고 변경 영역만 자동 재측정
...
- **자동 수정 도입 검토**: 현재 코드를 자동으로 고치는 hook은 운영하지 않습니다(잘못된 자동 수정의 회귀 비용이 사람이 직접 고치는 비용보다 커서 보류). `hasLength → hasText` 같은 대량 치환(183개 파일 307곳)은 일회성 스크립트 + 사람 검토로 수행했고 회귀가 없음을 확인했습니다. 이렇게 회귀 없음이 검증된 좁은 패턴부터 점진적으로 자동화 후보로 검토할 계획입니다.
- **외부 시스템 계약 카탈로그 자동 동기화**: 현재 사람 확인 게이트로 운영. 일부 외부 시스템은 API 스펙을 노출하므로 자동 동기화 검토.
- **CI 통합 강화**: 하네스 일관성 검증 7종은 이미 CI(Bitbucket Pipeline)에서 돌지만, 로컬 advisory hook(고객사 경계, 의심 패턴 등)의 검출 결과까지 PR 단계 리포트로 모으는 통합은 아직 검토 중.
- **운영 메트릭 정량화**: AI 코드 생성 비중 변화, 사람 리뷰 시간, drift 해결까지 걸리는 시간 등 ROI 지표 측정 시작.

### 8.3 다른 프로젝트 적용 가능성

본 시스템은 정책 YAML과 hook Python 코드가 프로젝트 특화 영역(고객사 매핑, 외부 시스템 enum 등)과 분리되어 있어, **골격(거버넌스 Tier 체계, 6단계 파이프라인, 카탈로그 형식)은 다른 프로젝트에 이식 가능**합니다. 다만 hook 코드 일부는 이 저장소의 경로·도메인 enum에 결합돼 있어 포팅 시 재작성이 필요하고, 아직 다른 저장소에 실제로 이식한 사례는 없는 **미검증 가설**입니다(§8.1 한계와 동일하게 정직히 적습니다).
...
**FAILURE_PATTERNS 75건 — 테마별 분해** (아래 10개 테마 합 = 75건, 카테고리 40여 종을 성격별로 묶음)

| 테마 | 건수 | 대표 카테고리 |
|------|------|--------------|
| 테스트 누락, 부채 | 17 | `test-gap` 14, `test-debt-no-ci-gate` 2, `stale-test` 1 |
| AI 작업 운영 실패 | 11 | `scope-miss` 6, `ai-hallucination` 3, `name-hallucination` 1, `hallucination` 1 |
| 코드 결함, 컨벤션 | 13 | `convention-drift` 5, `bug` 2, `production-logic-inverted` 1, `bean-wiring-runtime-bug` 1 외 |
| 도구, 환경 마찰 | 7 | `tool-ergonomics` 4, `environment-drift` 1, `local-dev-drift` 1, `git-hygiene` 1 |
| 설계 결정, 관측 | 5 | `design-decision` 3, `observer-pattern-incomplete` 1, `spec-drift` 1 |
| 보안 | 3 | `security` 2, `security-leak` 1 |
| 프로토타입 | 2 | `prototype-drift` 1, `prototype-leak-to-production` 1 |
| 외부 시스템 계약 | 2 | `external-contract-unverified` 1, `external-contract-drift` 1 |
| 의존성, 회귀 | 2 | `dependency-removal` 1, `regression` 1 |
| 기타 단발 (hook 오탐, 룰, 브랜치, 고객사 등) | 13 | `hook-false-positive` 2, `rule-update` 2, `branch-gap` 2, `cross-customer` 1 외 단발 |

> 대표 사고 예시: TDD 표방 vs Service 커버리지 1% · "완료" 선언 후 새 작업 반복 · `ExpectedFeeCalculator` NPE · 다중 세션 `index.lock` 충돌 · Nexus IP/JWT 하드코딩 · enum ↔ Jenkins 지원값 불일치(§4.2 사례 2) · Gson 제거 → transitive ClassNotFound · 공통 변경 시 고객사 0원 회귀
...
- 머지 후 자동 검증 보고서: 머지마다 1건씩 누적되는 시계열 (측정 시점에 따라 변동)
- 검증 자산 변동 스냅샷: 머지마다 누적되는 시계열
...
| # | 검사 | 검출 방법 | 출처 사례 |
|---|------|-----------|-----------|
| a | @Scheduled try-catch 누락 | 정규식 + 중괄호 매칭으로 try 블록 확인 | `@Scheduled` 42개 중 11개만 보호 (2026-04-17) |
| b1 | Enum.valueOf 래핑 누락 | `Enum.valueOf` + 주변 try-catch | enum 변환 IllegalArgument가 500으로 노출 |
| b2 | 빈 i18n 키 `getMessage("")` | `getMessage("")` 패턴 | i18n 동기화 실패로 빈 문자열 표시 |
| b3 | hasLength vs hasText 혼동 | `StringUtils.hasLength` 발견 시 경고 | 공백을 유효 입력으로 받아 NPE 발생 |
| c | 외부 enum origin (**비활성**) | rule 96 R1 폐기로 빈 결과 반환 (확인은 사람 게이트 R2/R3/R4) | enum ↔ Jenkins 불일치 (2026-04-23, §4.2 사례2) — 사람 게이트로 이관·retired |
| d | Flyway 중복/누락 채번 | 파일명 시퀀스 + 내용 비교 | 두 개발자가 같은 V2.1.x 번호로 충돌 |
| e | JS 한글 라벨 (i18n 미적용) | 따옴표 안 한글 리터럴 정규식 | 한글 라벨이 영어권 화면에 노출 |
| f | i18n bridge 객체 (`*_MSG`) | `_MSG`/`_MESSAGES` JS 객체 정규식 | JS 객체 i18n이 자동 동기화에서 누락 |
| g | DTO에 Entity implements 오용 | `class …(Row\|Dto) … implements Entity` | DTO가 Entity로 분류돼 JPA 충돌·로딩 실패 |
| h | i18n 키 중복 (같은 값 N곳) | 신규 키 value를 기존 맵과 대조 | 같은 메시지가 다른 키 N곳에 정의 |

> ※ "머지 후 검사"를 세는 단위 정리 — post-merge **hook 파일은 4개**, 그중 이 보고서 검사(`post_merge_incoming_review.py`) **1개** 안에 **검사 함수 8개**(a~h)가 있고, c가 비활성이라 **활성 검출은 7종**, 위 표는 b를 3패턴(b1~b3)으로 펼쳐 **10행**으로 보입니다. 4/7/8/10은 모두 같은 시스템을 다른 단위로 센 값입니다.
...
| 11 | Plugin boundary 위반 | 로컬 pre-commit advisory 출력 | 커밋마다 | 커밋 |
...
- **하네스 일관성 CI 검증** (운영 중): `bitbucket-pipelines.yml`의 한 step이 하네스 검증 스크립트 7종(verify_harness_consistency, verify_harness_surface, verify_skill_sync, check_doc_links, validate_claude_structure, validate_skill_toc, validate_agent_description)을 실행해, 위반이 있으면 파이프라인을 실패시킨다. 단 고객사 경계 검사(verify_customer_boundary.py)는 CI가 아니라 로컬 pre-commit advisory로 동작한다.
- **로컬 hook 결과의 CI 노출** (향후 계획): 로컬 advisory hook의 검출 결과까지 CI 리포트로 모으는 통합은 검토 중

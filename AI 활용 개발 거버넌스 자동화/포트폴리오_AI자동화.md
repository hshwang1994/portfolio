...
**거버넌스 핵심 원칙 3가지** (아래 2·3번은 위 표의 서로 다른 Tier 체계입니다):

- **자동과 수동의 경계를 정책 파일로 선언**합니다. 매번 사람이 판단하지 않고 `auto-fix-patterns.yaml`이 자동으로 분기시킵니다.
- **변경 등급 Tier 3(절대 자동 금지)** 은 자동 변경을 아예 시도하지 않습니다. 보호 경로 제거 같은 위험 변경은 사용자 명시 승인만 가능합니다.
...
#### 멀티에이전트 오케스트레이션 — 50개 작업자를 병렬로, 역할로 나눠

agent 50개는 각자 역할이 다른 전문 작업자입니다(백엔드 리팩토링, 경계 리뷰, 마이그레이션 등). 큰 작업이 들어오면 **메인 세션(오케스트레이터)이 서로 독립적인 일을 여러 작업자에게 동시에 맡깁니다.**

![멀티에이전트 오케스트레이션 — 병렬 디스패치와 역할 분업](AI자동화/49-multi-agent.png)
